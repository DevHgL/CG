...
int main() {
    ...
    const std::string cloud_path = "Images/graveyardtilesetnew/png/Objects/Cloud.png";
    ...
    ImageRGBA cloud{cloud_path};
    if (cloud.width() == 0) { std::cerr << "Erro: Imagem da nuvem não carregada: " << cloud_path << std::endl; return 1; }
    ...
    // 4. Adicionar Nuvens
    int cloud1_x = 100, cloud1_y = 50;
    over_scaled(background, cloud, cloud1_x, cloud1_y, scale);
    int cloud2_x = 500, cloud2_y = 80;
    over_scaled(background, cloud, cloud2_x, cloud2_y, scale);

    // 5. Adicionar os personagens (posicionados acima do chão)
